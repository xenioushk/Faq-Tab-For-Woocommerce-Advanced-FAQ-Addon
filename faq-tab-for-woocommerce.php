...
define('FAQTFW_ADDON_CURRENT_VERSION', '1.0.7'); // change plugin current version in here.

define('FAQTFW_DIR', plugin_dir_path(__FILE__));

require_once( FAQTFW_DIR . 'public/class-faqtfw-addon.php' );

register_activation_hook(__FILE__, array('BAF_faqtfw', 'activate'));
register_deactivation_hook(__FILE__, array('BAF_faqtfw', 'deactivate'));

add_action('plugins_loaded', array('BAF_faqtfw', 'get_instance'));
...
if (is_admin()) {
    ...
    add_action('plugins_loaded', array('BAF_faqtfw_Admin', 'get_instance'));
}